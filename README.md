...
> 最新版本：`v1.0.2` · bundle: `com.yinhe.staraudiobridge` · API 24 (HarmonyOS 6.1.1) · DevEco/hvigor 6.24
...
2. 手机与电脑同一 Wi-Fi:
   ...
   - 手动: 输入电脑 IP 连接
3. 开关「播放电脑声音」「手机当麦克风」:
   - 双向开启后可选择「防回声优先」或「音质优先」，切换时无需断开连接
   - 防回声优先（默认）：使用系统语音通话链路并优先走听筒，以启用 AEC、减少声学回授
   - 音质优先：电脑声音保持 48kHz 双声道音乐链路，手机麦克风使用普通采集；建议搭配耳机，手机外放可能产生回声
